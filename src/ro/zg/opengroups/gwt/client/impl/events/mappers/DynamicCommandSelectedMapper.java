...
import ro.zg.opengroups.gwt.shared.vo.OpenGroupsCommand;
...
public class DynamicCommandSelectedMapper implements UserEventToCommandMapper{
    public Command getCommandForUserEvent(UserEvent userEvent, ViewsManager viewsManager) {
	String commandId=(String)userEvent.removeEventParam(UserEventParams.COMMAND_ID);
	userEvent.addEventParam(OpenGroupsParams.ENTITY_ID, viewsManager.getCachedParam(OpenGroupsParams.ENTITY_ID));
//	Command command = Command.build(commandId, userEvent.getEventParams(), userEvent.getTargetViewId());
	Command command = new OpenGroupsCommand(commandId, userEvent.getEventParams(), userEvent.getTargetViewId());
	return command;
    }
}
